...
const modules: Array<{
  ...
  { id: 'wishlist', title: '种草图鉴', description: '集中查看想进一步了解的生物。', icon: Heart, tone: 'bg-rose-50 text-rose-600' },
  { id: 'care', title: '养护收藏', description: '出现问题时，快速找到收藏的处理步骤。', icon: BookOpenCheck, tone: 'bg-sky-50 text-sky-700' },
  { id: 'memorial', title: '生命纪念', description: '保留离缸日期与养护复盘。', icon: Skull, tone: 'bg-slate-100 text-slate-600' },
  { id: 'achievements', title: '成就勋章', description: '查看自动解锁的守护记录与下一步。', icon: Medal, tone: 'bg-amber-50 text-amber-700' },
];
      ...
      <header className="overflow-hidden rounded-[28px] border border-white/80 bg-[linear-gradient(135deg,#ffffff_0%,#edf7f1_58%,#dfeee8_100%)] p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[12px] font-bold text-ink/48">种草 · 养护 · 纪念 · 勋章</p>
          </div>
            ...
            >
              ...
              <span className="flex min-w-0 flex-1 flex-col self-stretch">
                <span className="flex items-start justify-between gap-3">
                  <span className="text-[18px] font-black text-ink">{item.title}</span>
                  <ChevronRight className="h-5 w-5 shrink-0 text-ink/25 transition-transform group-hover:translate-x-0.5" />
                </span>
                <span className="mt-2 text-[12px] font-bold leading-5 text-ink/48">{item.description}</span>
