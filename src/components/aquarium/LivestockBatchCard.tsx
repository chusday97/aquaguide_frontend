...
const lifeStageOptions: Array<{ value: LifeStage; zh: string; en: string }> = [
  { value: 'unknown', zh: '未确认', en: 'Unknown' },
  { value: 'juvenile', zh: '幼年', en: 'Juvenile' },
  { value: 'adult', zh: '成年', en: 'Adult' },
];

const reproductiveOptions: Array<{ value: ReproductiveState; zh: string; en: string }> = [
  { value: 'unknown', zh: '未确认', en: 'Unknown' },
  { value: 'normal', zh: '常态', en: 'Normal' },
  { value: 'pregnant_or_gravid', zh: '怀孕 / 抱卵', en: 'Pregnant / gravid' },
  { value: 'in_labor_or_spawning', zh: '生产 / 繁殖中', en: 'Birthing / spawning' },
  { value: 'postpartum_recovery', zh: '产后恢复', en: 'Postpartum recovery' },
];
      ...
      <Dialog open={open} onOpenChange={next => {
        ...
        <DialogContent className="max-h-[88dvh] max-w-3xl overflow-hidden rounded-[28px] p-0">
          <DialogHeader className="border-b border-border px-5 pb-4 pt-5">
            ...
            <DialogDescription>{isEn ? 'Use groups for fish that entered together or now have different life or reproductive states.' : '同批入缸或体态相同的生物放在一组，不需要给每条鱼命名。'}</DialogDescription>
          </DialogHeader>
          <div className="app-scrollbar-hidden min-h-0 overflow-y-auto px-4 py-4 md:px-5">
              ...
              {batches.map((batch, index) => (
                <section key={batch.id} className="rounded-[20px] border border-border/80 bg-bg/55 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-sm font-black text-ink"><Baby className="h-4 w-4 text-emerald-700" />{isEn ? `Group ${index + 1}` : `第 ${index + 1} 组`}</div>
                    <div className="flex gap-1">
                      {batch.quantity > 1 && <button type="button" onClick={() => { setSplitSource(batch); setSplitQuantity(1); }} className="flex h-10 items-center gap-1 rounded-xl px-2 text-xs font-black text-emerald-700 hover:bg-emerald-50"><GitBranch className="h-4 w-4" />{isEn ? 'Split' : '拆分'}</button>}
                      <button type="button" onClick={() => setPendingDelete(batch)} aria-label={isEn ? `Delete group ${index + 1}` : `删除第 ${index + 1} 组`} className="flex h-10 w-10 items-center justify-center rounded-xl text-rose-600 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <label className="text-xs font-black text-ink/55">{isEn ? 'Quantity' : '数量'}<input type="number" min={1} value={batch.quantity} onChange={event => update(batch.id, { quantity: Math.max(1, Number(event.target.value) || 1) })} className="mt-1 h-11 w-full rounded-xl border border-border bg-white px-3 text-sm text-ink" /></label>
                    <label className="text-xs font-black text-ink/55">{isEn ? 'Entry date' : '入缸日期'}<input type="date" value={batch.entryDate.slice(0, 10)} onChange={event => update(batch.id, { entryDate: new Date(`${event.target.value}T00:00:00`).toISOString() })} className="mt-1 h-11 w-full rounded-xl border border-border bg-white px-3 text-sm text-ink" /></label>
                    <label className="text-xs font-black text-ink/55">{isEn ? 'Life stage' : '生长阶段'}<select value={batch.lifeStage} onChange={event => update(batch.id, { lifeStage: event.target.value as LifeStage })} className="mt-1 h-11 w-full rounded-xl border border-border bg-white px-3 text-sm text-ink">{lifeStageOptions.map(option => <option key={option.value} value={option.value}>{isEn ? option.en : option.zh}</option>)}</select></label>
                    {reproductiveApplicable && <label className="text-xs font-black text-ink/55">{isEn ? 'Reproductive state' : '繁殖状态'}<select value={batch.reproductiveState === 'not_applicable' ? 'unknown' : batch.reproductiveState} onChange={event => update(batch.id, { reproductiveState: event.target.value as ReproductiveState })} className="mt-1 h-11 w-full rounded-xl border border-border bg-white px-3 text-sm text-ink">{reproductiveOptions.map(option => <option key={option.value} value={option.value}>{isEn ? option.en : option.zh}</option>)}</select></label>}
                  </div>
                </section>
              ))}
            </div>
            ...
            {splitSource && (
              <section className="mt-4 rounded-[20px] border border-amber-200 bg-amber-50 p-4">
                <div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-black text-amber-950">{isEn ? 'Split into a new group' : '拆出一组'}</h3><p className="mt-1 text-xs font-semibold text-amber-900/60">{isEn ? `Current group: ${splitSource.quantity}` : `原组共 ${splitSource.quantity} 条/只`}</p></div><button type="button" onClick={() => setSplitSource(null)} className="flex h-10 w-10 items-center justify-center rounded-xl text-amber-900/60"><X className="h-4 w-4" /></button></div>
                <div className="mt-3 grid gap-3 sm:grid-cols-3">
                  <label className="text-xs font-black text-amber-950/70">{isEn ? 'Move quantity' : '拆出数量'}<input type="number" min={1} max={Math.max(1, splitSource.quantity - 1)} value={splitQuantity} onChange={event => setSplitQuantity(Number(event.target.value) || 1)} className="mt-1 h-11 w-full rounded-xl border border-amber-200 bg-white px-3" /></label>
                  <label className="text-xs font-black text-amber-950/70">{isEn ? 'Life stage' : '生长阶段'}<select value={splitLifeStage} onChange={event => setSplitLifeStage(event.target.value as LifeStage)} className="mt-1 h-11 w-full rounded-xl border border-amber-200 bg-white px-3">{lifeStageOptions.map(option => <option key={option.value} value={option.value}>{isEn ? option.en : option.zh}</option>)}</select></label>
                  {reproductiveApplicable && <label className="text-xs font-black text-amber-950/70">{isEn ? 'Reproductive state' : '繁殖状态'}<select value={splitReproductiveState} onChange={event => setSplitReproductiveState(event.target.value as ReproductiveState)} className="mt-1 h-11 w-full rounded-xl border border-amber-200 bg-white px-3">{reproductiveOptions.map(option => <option key={option.value} value={option.value}>{isEn ? option.en : option.zh}</option>)}</select></label>}
                </div>
                <button type="button" onClick={confirmSplit} className="mt-3 min-h-11 rounded-xl bg-amber-900 px-4 text-sm font-black text-white"><Plus className="mr-1 inline h-4 w-4" />{isEn ? 'Confirm split' : '确认拆分'}</button>
              </section>
            )}
