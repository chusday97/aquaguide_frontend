...
- 第一版状态判断只适用于鱼类规则集；水草、硬景、珊瑚和其他非鱼类候选可以查看资料，但不能进入“鱼只呼吸/姿态”问诊。
- 用户确认鱼类后，本地展示层把物种需求与当前鱼缸水体、温度、空间、过滤、增氧、近期新增和换水转换为受控观察；这些观察与自由描述、结构化回答共同进入同一确定性排序。
- 连续或多条死亡属于本地红旗，必须在继续追问前展示增氧、检查过滤、暂停喂食/加药和检查水质等安全动作。
- AI 只补充受控观察代码和值；非法值被过滤，环境不匹配、紧急等级、原因集、问题收益、动作和文章白名单均由本地规则决定。
- 鱼缸页的鱼类异常入口必须进入 `/identify`，不得再提供基于关键词拼接答案的第二套公开追问；水质、设备、换水、虾类和水草继续使用各自确定性检查。
