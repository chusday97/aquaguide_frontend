...
巡检不得重新拆成逐题页面。六个必答观察项与一个可选描述同页显示，回答完整后一次进入结果；进度只表示已回答数量。
...
手机端种草与养护收藏的正式入口统一进入 `/collection/wishlist` 与 `/collection/care`，不再在鱼缸、图鉴或养护页展开第二套收藏列表。水族册首页只显示四张模块卡；桌面侧栏二级标题直接进入四个独立地址。收藏成功提示“已收录到水族册”，并提供一次“去水族册查看”。
...
- 只有异常和混养风险允许用户主动请求 AI 简短解释；AI 不改变行动类型或优先级。
