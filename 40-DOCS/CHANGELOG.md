...
- **全站中英文双语 100% 完全对齐与全量 JSX 重构**：进行了全项目 83 个源文件的 AST 与正则深度扫描，排查出 265 处直接嵌入在 JSX 文本节点与组件属性中的硬编码中文；完成了 `Aquarium.tsx`（常用操作/水质参数/设备分类/数据提醒）、`SpeciesDetailDialog`（系统适配结果/喂养与养护）、`CompatibilityRiskCalculator`（混养计算/清除组合/已选生物/匹配结论）、`Home.tsx`（养鱼图鉴/AI 助手/水温/pH/换水/体型/饮食）、`Login.tsx`（登录/邮箱/密码）、`AIAssistant.tsx`（结论/推理/下一步）及 `Encyclopedia.tsx` 等 7 个核心页面/组件的全量 `{isEn ? '...' : '...'}` 条件重构，确保在英文模式下全站界面无任何遗漏的中文节点。
