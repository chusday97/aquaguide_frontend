...
- **养护百科全量汉化与中英双语切换修复**：全面本地化 `CareEncyclopedia.tsx` 内硬编码的中文规则、步骤、判断依据、卡片标签及自查诊断结论引擎；补充生成过水、水质恶化、鱼苗照料、安全换水、死鱼处理等 5 张英文养护图的 480w / 960w 高清 WebP 响应式图片，解决英文视角下 `[?]` 掉图问题；新增英文诊断问题预设库 `stepDiagnosisQuestionsEn`、答案标签映射 `answerLabelMapEn`、推荐原因本地化 `getRecommendationReasonLocalized` 及物种标签本地化 `translateTopicTag`；确保在英文语言状态下，顶栏摘要 (`Based on:...`)、推荐原因标签、自查问答流程、紧急动作卡、避坑提醒及动态生成的诊断诊断卡实现 100% 全量纯英文显示，无中英混杂。
