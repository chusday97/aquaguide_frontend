...
- **全站中英文双语 100% 完全对齐与补全**：进行了全项目 83 个源文件的静态词条深度扫描；补全了 `SpeciesDetailDialog`（水体/温度/水质/缸体大小/养护难度/过滤/加热/混养）、`CompatibilityRiskCalculator`（冲突分类/调整建议/弹窗标题）、`TemplatePlanCard`（缸型/尺寸/环境/造景/生物）、`HeaterReminderCard`（本地温控/判断中/恒温提示）及 `visual-result.adapters.ts`（混养风险类别英译）等残留中文标签，确保在英文模式下全站弹窗、卡片、摘要与提示达到 100% 纯净对应的英文呈现。
