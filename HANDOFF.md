...
- 当前阶段：核心体验已收口，正在实施 React 前端、Express 业务 API 与 Supabase PostgreSQL/Storage 三层拆分。
...
- 当前步骤：字段契约、API workspace、用户业务 API、Repository 边界、内容管理 API、批量导入和简易后台页面均已完成；下一步先锁定迁移 commit 的请求载荷，再实施游客迁移/离线队列并逐页接入。
...
1. 复核导入预检发现的 93 组跨内容重复素材，迁移前确认哪些是合法共图、哪些需返工。
2. 先确认迁移提交是“客户端重传预览载荷”还是“服务端暂存预览载荷”，再实现预览/确认与离线待同步队列。
3. 按内容只读、收藏、鱼缸和记录顺序接入现有页面，每个页面单独验收。
