...
- 当前步骤：国际化翻译表、RLS、API 回退、语言偏好接口、应用壳词条和桌面/手机设置入口已完成；下一步补用户端业务词条、英文草稿审核后台与登录偏好同步。
...
| 2026-07-16 | 中英文语言契约、设置入口、内容回退和手机养护轮播宽度修复 | commit `d542fd0`；lint/API check/build；24 表契约；API 边界；双语桌面/手机与养护浏览器回归 |
...
| 英文内容不能正式开放 | 已完成翻译表、API 回退、应用壳词条和语言选择 | 486 个物种与 41 篇文章尚未完成初译校验和人工审核 | 覆盖率达到 100%，管理员审核发布后再解除正式入口门禁 |

## 下一步计划

1. 按鱼缸、图鉴、养护、水族册、登录与错误提示顺序补齐用户端 i18next 词条，并持续验证长英文折行。
2. 增加英文草稿生成、术语与数值校验、管理员对照审核和覆盖率门禁；未审核内容不得发布。
...
- 当前预览：Web 为 `http://localhost:3000`，API 为 `http://localhost:8787`；本轮开发实例已启动并完成页面与接口验证。
