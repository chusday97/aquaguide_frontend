...
begin
  ...
  select id into species_uuid from public.species where catalog_key = target_species_catalog_key;
  insert into public.memorial_records (
    id, owner_id, aquarium_id, species_id, species_catalog_key, memorial_date, reason
  ) values (
    new_memorial_id, auth.uid(), target_aquarium_id, species_uuid, target_species_catalog_key, target_memorial_date, target_reason
...
revoke all on function public.record_livestock_memorial(uuid, uuid, uuid, integer, text, date, text, uuid) from public;
grant execute on function public.record_livestock_memorial(uuid, uuid, uuid, integer, text, date, text, uuid) to authenticated;
